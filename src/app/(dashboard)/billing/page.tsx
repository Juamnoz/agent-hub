...
export default function BillingPage() {
  ...
  return (
    <div className="space-y-5 pb-4">
      ...
      {/* Current plan banner */}
      <div className="rounded-2xl bg-card ring-1 ring-orange-200 dark:ring-orange-500/30 shadow-[0_1px_3px_rgba(0,0,0,0.06)] overflow-hidden">
        <div className="flex items-center justify-between px-4 pt-4 pb-3">
          <div>
            <p className="text-[12px] font-medium text-muted-foreground">{t.billing.currentPlanBadge}</p>
            <div className="flex items-baseline gap-2 mt-0.5">
              <h2 className="text-[22px] font-bold text-foreground">Pro</h2>
              <span className="text-[13px] text-muted-foreground">$80 {t.billing.perMonth}</span>
            </div>
          </div>
          <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-orange-50 dark:bg-orange-500/15">
            <Star className="h-6 w-6 text-orange-500" />
          </div>
        </div>
        {/* Usage mini-bars */}
        <div className="px-4 pb-4 space-y-2.5 border-t border-border/60 pt-3 bg-muted/20">
          <UsageBar label="3 / 3 agentes" pct={100} color="bg-orange-400" track="bg-orange-100 dark:bg-orange-500/20" />
          <UsageBar label="124 / 3,000 conversaciones" pct={4} color="bg-orange-400" track="bg-orange-100 dark:bg-orange-500/20" />
          <UsageBar label="2 / 5 WhatsApp" pct={40} color="bg-orange-400" track="bg-orange-100 dark:bg-orange-500/20" />
        </div>
      </div>
...
function UsageBar({ label, pct, color, track }: { label: string; pct: number; color: string; track: string }) {
  return (
    <div>
      <span className="text-[12px] text-muted-foreground">{label}</span>
      <div className={`mt-1 h-1.5 rounded-full ${track}`}>
        <div
